...
friend inline Interval sqr (const Interval &x){
  ...
  friend inline bool isinf(const Interval& r) {
	return r.getBaseInterval().isInfinite();
  }
  ///  returns true if any of endpoints is a NaN
  friend inline bool isnan(const Interval& r) {
	return (isnan(r.leftBound()) or isnan(r.rightBound()));
  }
  /// returns interval containing copysign(a, b) for each a in x and b in y
  friend inline Interval copysign(const Interval &x, const Interval &y){
	if(y.contains(0.0)) {
	  BoundType m = max(-x.leftBound(), x.rightBound());
	  return Interval(-m, m);
	} else {
	  return y<0 ? -x : x;
	}
...
public:
  ...
  /// an absolute value
  static inline IntervalType abs (const IntervalType & A_inter)
  {
	return IntervalType(A_inter.abs());
  } // abs

  ///maximum
  static inline IntervalType max (
	  const IntervalType & A_iv1,
...
///minimum
  static inline IntervalType min (const IntervalType& A_iv1,
									   const IntervalType& A_iv2)
  {
	return IntervalType(
		(A_iv1.leftBound()<A_iv2.leftBound() ? A_iv1.leftBound() : A_iv2.leftBound()),
