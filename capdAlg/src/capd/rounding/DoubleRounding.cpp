...
#elif CAPD_CPU_ARCH==CAPD_CPU_ARCH_ARM64

#define ARM64_DEFAULT_FPU_FLAGS 0x0000000
static unsigned long ROUND_NEAREST_CW = ARM64_DEFAULT_FPU_FLAGS;
static unsigned long ROUND_DOWN_CW    = ARM64_DEFAULT_FPU_FLAGS | 0x800000;
static unsigned long ROUND_UP_CW      = ARM64_DEFAULT_FPU_FLAGS | 0x400000;
static unsigned long ROUND_CUT_CW     = ARM64_DEFAULT_FPU_FLAGS | 0xC00000;

void DoubleRounding::roundNearest(void){
    asm volatile ("msr fpcr, %x[flags]" :: [flags] "r" (ROUND_NEAREST_CW));
}

void DoubleRounding::roundDown(void){
   asm volatile ("msr fpcr, %x[flags]" :: [flags] "r" (ROUND_DOWN_CW));
}

void DoubleRounding::roundUp(void){
    asm volatile ("msr fpcr, %x[flags]" :: [flags] "r" (ROUND_UP_CW));
}

void DoubleRounding::roundCut(void){
    asm volatile ("msr fpcr, %x[flags]" :: [flags] "r" (ROUND_CUT_CW));
}
